...
def create_main_telemetry(u_df, r_df):
    ...
    # Delta
    fig.add_trace(go.Scatter(x=u_df['distance'], y=u_df['delta'], name="Delta", fill='tozeroy', line=dict(color='white')), row=2, col=1)

    # Throttle
    fig.add_trace(go.Scatter(x=u_df['distance'], y=u_df['throttle']*100, name="Throttle", line=dict(color='green')), row=3, col=1)

    # Brake
    fig.add_trace(go.Scatter(x=u_df['distance'], y=u_df['brake']*100, name="Brake", line=dict(color='red')), row=4, col=1)
