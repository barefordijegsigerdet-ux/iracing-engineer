...
st.set_page_config(page_title="Race Engineer Pro | Master v3.9", layout="wide")

def clean_df(df):
    df.columns = df.columns.str.lower().str.replace(' ', '').str.replace('_', '')
    mapping = {
        'dist': ['dist', 'lapdist', 'distance', 'lapdistpct'],
        'time': ['time', 'sessiontime', 'laptime', 'elapsed'],
...
def main():
    ...
    with st.sidebar:
        ...
        if d_file == b_file:
            st.error("⚠️ SELECT DIFFERENT LAPS")
            st.stop()
    ...
    if res_d['time'].max() > 0 and res_d['time'].max() != res_b['time'].max():
        ...
        st.sidebar.success("⏱️ Time-Sync: ACTIVE")
    else:
    ...
    # Cast to float to ensure formatting works
    final_delta = float(delta.iloc[-1]) if hasattr(delta, 'iloc') else float(delta[-1])
    st.metric("Total Lap Delta", f"{final_delta:.3f}s", delta_color="inverse")
    ...
    for eid in events.unique():
        ...
        # FIX: grid[idx] is a numpy array. Use standard indexing, not .iloc
