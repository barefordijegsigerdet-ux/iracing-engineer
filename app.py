...
# --- UI ---
st.title("🏎️ iRacing Telemetry: Separate Analysis")
...
if df_ref is not None and df_user is not None:
    ...
    # 3. THROTTLE
    fig.add_trace(go.Scatter(x=data['dist'], y=data['ref_throttle']*100, name="Leeroy Gas", line=dict(color=COLOR_LEEROY, width=1, dash='dot')), row=3, col=1)
    fig.add_trace(go.Scatter(x=data['dist'], y=data['user_throttle']*100, name="Jonas Gas", line=dict(color=COLOR_JONAS, width=2)), row=3, col=1)

    # 4. BRAKE
    fig.add_trace(go.Scatter(x=data['dist'], y=data['ref_brake']*100, name="Leeroy Brems", line=dict(color=COLOR_LEEROY, width=1, dash='dot')), row=4, col=1)
    fig.add_trace(go.Scatter(x=data['dist'], y=data['user_brake']*100, name="Jonas Brems", fill='tozeroy', line=dict(color='rgba(255, 0, 0, 0.3)', width=2)), row=4, col=1)
    ...
    fig.add_trace(go.Scatter(x=data['dist'], y=data['user_gear'], name="Jonas Gear", line=dict(color='orange', width=2, shape='hv')), row=5, col=1)

    # 6. STEERING
    fig.add_trace(go.Scatter(x=data['dist'], y=data['ref_steer'], name="Leeroy Rat", line=dict(color=COLOR_LEEROY, width=1, dash='dot')), row=6, col=1)
    fig.add_trace(go.Scatter(x=data['dist'], y=data['user_steer'], name="Jonas Rat", line=dict(color=COLOR_JONAS, width=2)), row=6, col=1)
